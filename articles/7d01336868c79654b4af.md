...
## 概要
仕事でNuxtとLaravelでSPAを構築する機会があったため開発環境構築方法を備忘録として残しておく
...
同一リポジトリ環境下になりますが、[Sanctumを利用する場合はこちらの記事を参考にしてください](https://zenn.dev/nagi125/articles/78d931f9b72fff1f1a7d)
...
#### プロジェクトの作成
「docker-compose」で始まるコマンドはコンテナ内で実行しており、その記述がない
ファイルがあるとcurrentにプロジェクトを作成できないため「blog」とする。
```
...
ファイルが存在し、currentに作れなかったためcurrentに移動させる
```
$ cp -R blog/. ./
$ rm -r blog
```
